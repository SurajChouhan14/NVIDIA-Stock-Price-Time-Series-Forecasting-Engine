...
3. Holt's Linear Exponential Smoothing with automated parameter estimation (statsmodels.tsa.holtwinters.Holt)
...
class NVDATimeSeriesForecaster:
    ...
    def run_adf_stationarity_test(self, series=None):
        ...
        # Execute statsmodels ADF test with AIC lag selection
    ...
    def fit_and_forecast_holt(self, optimized=True, alpha=None, beta=None, forecast_horizon=60):
        """
        Holt's Linear Exponential Smoothing with trend component.
        By default, optimizes smoothing parameters (alpha, beta) via MLE / sum of squared errors.
        """
        model = Holt(self.train_prices, initialization_method='estimated')
        if optimized:
            fitted_model = model.fit(optimized=True)
        else:
            fitted_model = model.fit(smoothing_level=alpha or 0.30, smoothing_trend=beta or 0.10)
    ...
    def fit_and_forecast_arima(self, p=2, d=1, q=1, forecast_horizon=60):
        ...
        Integrates forecasts back into dollar price levels via exponential transform.
        """
    ...
    def evaluate_forecast(self, forecasts, actuals=None):
        ...
        # Note: Compares predicted one-step change relative to previous actual price level
